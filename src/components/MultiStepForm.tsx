...
export default function MultiStepForm() {
    ...
    const nextStep = () => setStep((prev) => (prev < steps.length ? prev + 1 : prev)); console.log(step);
    ...
    return (
        <div>
            <div className="bg-white dark:bg-dark-900 py-24 transition-shadow">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="flex justify-center items-center mt-10">
                        <div className="bg-white  shadow-lg rounded-lg p-4 max-w-5xl min-h-[45.5rem] w-full flex ">
                            {/* Sidebar */}
                            <div className="w-1/3  Steps bg-[#00477f] text-white p-6 rounded-lg">
                                {steps.map((label, index) => (
                                    <div key={index} className="flex items-center  font-semibold mb-4">
                                        <div
                                            className={`w-8 h-8 min-h-8 min-w-8 flex items-center justify-center rounded-full ${step === index + 1 ? "bg-[#eab308]" : "border border-white"
                                                }`}
                                        >
                                            {index + 1}
                                        </div>
